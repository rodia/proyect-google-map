...
$struct = array(
	...
	"collision_point" => array("latitude","longitude","MatchType","Address","City","State","Field1","Field2","Road1","Road2"),
	"detail_point" => array("LATITUDE","LONGITUDE","MatchType","Address","collision_id","Code","City","St1","St2","State")
);

$file = fopen("table-{$table}{$index}.csv", "r") or exit("Unable to open file!");
...
while (!feof($file))
{
	$line = explode($separator, fgets($file));
	if (++$key == 0) continue;
